...
from scripts.columns import ACCESSION, SEQUENCE
...
def extract_receptors(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Uniqueness key: (Accession, Sequence).
    ...
    Raises:
        ValueError: if any receptor row is missing a Accession or a Sequence.
    ...
    """
    receptor_group = df["Receptor"]
    assert isinstance(receptor_group, pd.DataFrame)
    rec: pd.DataFrame = receptor_group[list(_RECEPTOR_COLS.keys())].copy()
    ...
    missing_uniprot = rec[ACCESSION].isna()
    if missing_uniprot.any():
        ...
        msg = f"{n} receptor row(s) are missing a 'UniProt ID' (data row index: {bad_rows})"
    ...
    if missing_seq.any():
        ...
        msg_0 = (
            f"{n} receptor row(s) are missing a Sequence (data row index: {bad_rows})"
        )
        raise ValueError(msg_0)

    # --- Deduplication ---
    # Same (ACCESSION, SEQUENCE) → same receptor/variant
    unique_receptors: pd.DataFrame = (
        rec.drop_duplicates(subset=[ACCESSION, SEQUENCE])
